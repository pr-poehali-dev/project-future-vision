...
const tariffs = [
  { name: "Старт", amount: "2 000 ₽", daily: "0,5% — 1,5%", color: "border-primary/50", badge: "" },
  { name: "Стандарт", amount: "5 000 ₽", daily: "1% — 2%", color: "border-primary", badge: "Популярный" },
  { name: "Премиум", amount: "10 000 ₽", daily: "1,5% — 2,5%", color: "border-primary/80", badge: "Максимум" },
];

const periods = [
  { days: 14, label: "14 дней", bonus: "базовый тариф", bonusPercent: 0, weekend: "0,5% в выходные", desc: "Оптимально для первого входа" },
  { days: 28, label: "28 дней", bonus: "+1% к тарифу", bonusPercent: 1, weekend: "0,5% в выходные", desc: "Сбалансированный доход" },
  { days: 42, label: "42 дня", bonus: "+2% к тарифу", bonusPercent: 2, weekend: "0,5% в выходные", desc: "Максимальная прибыль" },
];
...
export default function Landing() {
  ...
  return (
    <main className="min-h-screen bg-background dark">
      {/* Nav */}
      <nav className="absolute top-0 right-0 z-20 p-6 flex gap-4">
        <Link to="/login" className="text-muted-foreground text-sm hover:text-primary transition-colors">
          Войти
        </Link>
        <Link to="/register" className="bg-primary text-primary-foreground px-4 py-2 text-sm hover:bg-primary/90 transition-all">
          Открыть счёт
        </Link>
      </nav>
      ...
      <section id="tariffs" className="py-24 px-6 bg-card/50">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-16">
            {tariffs.map((t, i) => (
              ...
              >
                ...
                {selectedTariff === i && <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-3/4 h-0.5 bg-primary" />}
              </div>
            ))}
          </div>
          ...
          <div className="relative p-8 md:p-10 border border-primary/30 bg-card text-center max-w-2xl mx-auto">
            ...
            <Link
              to="/register"
              className="inline-block mt-6 bg-primary text-primary-foreground px-8 py-3 text-sm uppercase tracking-wider hover:bg-primary/90 transition-all"
            >
              Открыть счёт
            </Link>
          </div>
      ...
      <section className="py-24 px-6 bg-card/50">
        <div className="max-w-4xl mx-auto">
          <div className="relative text-center py-12">
            <div className="absolute top-0 left-1/2 -translate-x-1/2 text-primary/20 font-serif text-9xl leading-none">&ldquo;</div>
      ...
      <section className="py-24 px-6 relative overflow-hidden">
        <div className="absolute inset-0 opacity-5"><ArtDecoSunburst /></div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="flex gap-6 text-sm">
              <Link to="/login" className="text-muted-foreground/50 hover:text-primary transition-colors">Войти</Link>
              <Link to="/register" className="text-muted-foreground/50 hover:text-primary transition-colors">Регистрация</Link>
            </div>
